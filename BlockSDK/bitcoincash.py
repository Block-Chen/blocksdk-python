...
class BitcoinCash(Base):
	...
	def getWallets(self,request = {}):
		...
		return self.request("GET","/bch/wallets",{
			"offset" : request['offset'],
			"limit" : request['limit']
		})
	...
	def getWalletAddress(self,request = {}):
		...
		return self.request("GET","/bch/wallets/" + str(request['wallet_id']) + "/addresses",{
	...
	def createWalletAddress(self,request = {}):
		...
		return self.request("POST","/bch/wallets/" + str(request['wallet_id']) + "/addresses",{
			"wif" : request['wif'],
			"password" : request['password']
		})
	...
	def sendToAddress(self,request = {}):
		...
		return self.request("POST","/bch/wallets/" + str(request['wallet_id']) + "/sendtoaddress",{
			"address" : request['address'],
	...
	def sendMany(self,request = {}):
		...
		return self.request("POST","/bch/wallets/" + str(request['wallet_id']) + "/sendmany",{
			"to" : request['to'],
			"wif" : request['wif'],
			"password" : request['password']
		})
