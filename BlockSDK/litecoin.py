...
class Litecoin(Base):
	def getBlockChain(self,request = {}):
		return self.request("GET","/ltc/info")
	...
	def getBlock(self,request = {}):
		...
		return self.request("GET","/ltc/blocks/" + str(request['block']),{
			"rawtx" : request['rawtx'],
	...
	def getMemPool(self,request = {}):
		...
		return self.request("GET","/ltc/mempool",{
			"rawtx" : request['rawtx'],
			"offset" : request['offset'],
			"limit" : request['limit']
		})
	...
	def getAddressInfo(self,request = {}):
		...
		return self.request("GET","/ltc/addresses/{request['address']}",{
	...
	def getAddressBalance(self,request = {}):
		return self.request("GET","/ltc/addresses/{request['address']}/balance")
	...
	def getWallets(self,request = {}):
		...
		return self.request("GET","/ltc/wallets",{
			"offset" : request['offset'],
			"limit" : request['limit']
		})

	def getWallet(self,request = {}):
		return self.request("GET","/ltc/wallets/{request['address']}")
	
	def createHdWallet(self,request = {}):
		if not('name' in request) or not request['name']:
			request['name'] = None
		
		return self.request("POST","/ltc/wallet/hd",{
			"name" : request['name']
		})
	
	def loadWallet(self,request = {}):

		return self.request("POST","/ltc/wallets/" + str(request['wallet_id']) + "/load",{
			"wif" : request['wif'],
			"password" : request['password']
		})

	def unloadWallet(self,request = {}):
		return self.request("POST","/ltc/wallets/" + str(request['wallet_id']) + "/unload")
	...
	def getWalletAddress(self,request = {}):
		...
		return self.request("GET","/ltc/wallets/" + str(request['wallet_id']) + "/addresses",{
	...
	def createWalletAddress(self,request = {}):
		...
		return self.request("POST","/ltc/wallets/" + str(request['wallet_id']) + "/addresses",{
			"wif" : request['wif'],
			"password" : request['password']
		})
	
	def getWalletBalance(self,request = {}):	
		return self.request("GET","/ltc/wallets/" + str(request['wallet_id']) + "/balance")		
	...
	def getWalletTransactions(self,request = {}):
		...
		return self.request("GET","/ltc/wallets/" + str(request['wallet_id']) + "/transaction",{
			"type" : request['type'],
			"order" : request['order'],
			"offset" : request['offset'],
			"limit" : request['limit']
	...
	def sendToAddress(self,request = {}):
		...
		return self.request("POST","/ltc/wallets/" + str(request['wallet_id']) + "/sendtoaddress",{
			"address" : request['address'],
	...
	def sendMany(self,request = {}):
		...
		return self.request("POST","/ltc/wallets/" + str(request['wallet_id']) + "/sendmany",{
			"to" : request['to'],
			"wif" : request['wif'],
			"password" : request['password']
		})
	...
	def sendTransaction(self,request = {}):
		return self.request("POST","/ltc/transactions/send",{
			"hex" : request['hex']
		})
	
	def getTransaction(self,request = {}):
		return self.request("GET","/ltc/transactions/" + str(request['hash']) + "")
