...
class Litecoin(Base):
	...
	def getBlock(self,request = {}):
		...
		return self.request("GET","/ltc/block/" + str(request['block']) ,{
			"rawtx" : request['rawtx'],
	...
	def getAddressInfo(self,request = {}):
		...
		return self.request("GET","/ltc/address/" + str(request['address']) + "",{
	...
	def getAddressBalance(self,request = {}):
		return self.request("GET","/ltc/address/" + str(request['address']) + "/balance")
	...
	def listWallet(self,request = {}):
		...
		return self.request("GET","/ltc/wallet",{
			"offset" : request['offset'],
			"limit" : request['limit']
		})
	...
	def createWallet(self,request = {}):
		...
		return self.request("POST","/ltc/wallet",{
			"name" : request['name']
		})
	...
	def loadWallet(self,request = {}):
		...
		return self.request("POST","/ltc/wallet/" + str(request['wallet_id']) + "/load",{
	...
	def unLoadWallet(self,request = {}):
		...
		return self.request("POST","/ltc/wallet/" + str(request['wallet_id']) + "/unload")
	...
	def listWalletAddress(self,request = {}):
		...
		return self.request("GET","/ltc/wallet/" + str(request['wallet_id']) + "/address",{
	...
	def createWalletAddress(self,request = {}):
		...
		return self.request("POST","/ltc/wallet/" + str(request['wallet_id']) + "/address",{
			"seed_wif" : request['seed_wif'],
			"password" : request['password']
		})
	...
	def getWalletBalance(self,request = {}):
		...
		return self.request("GET","/ltc/wallet/" + str(request['wallet_id']) + "/balance")
	...
	def getWalletTransaction(self,request = {}):
		...
		return self.request("GET","/ltc/wallet/" + str(request['wallet_id']) + "/transaction",{
	...
	def sendToAddress(self,request = {}):
		...
		return self.request("POST","/ltc/wallet/" + str(request['wallet_id']) + "/sendtoaddress",{
			"address" : request['address'],
	...
	def sendMany(self,request = {}):
		...
		return self.request("POST","/ltc/wallet/" + str(request['wallet_id']) + "/sendmany",{
			"to" : request['to'],
			"seed_wif" : request['seed_wif'],
			"password" : request['password']
		})
	...
	def sendTransaction(self,request = {}):
		return self.request("POST","/ltc/transaction",{
			"sign_hex" : request['sign_hex']
		})
	
	
	def getTransaction(self,request = {}):	
		return self.request("GET","/ltc/transaction/" + str(request['hash']) + "")
# litecoin = Litecoin('ad38QFTevz8fEEAG4fKsf4T5L8pwqgcy6LXMHpqU')
# print(litecoin.getBlock({
# 	"block" : 1520000,
#     "rawtx" :True,
#     "offset" :0,
#     "limit" :10
